...
# Uitgang detecteren en spiegelen
...
# Chromium kiosk naar lokaal bestand
exec ${CHROMIUM_BIN} \\
...
========================================
 Klaar!
 - Lokale pagina: file://${INDEX_FILE}
 - Fullscreen kiosk: ${CHROMIUM_BIN} --kiosk
 - Spiegeling: --reflect ${REFLECT}  (wijzig via: sudo REFLECT=y ./install_kiosk.sh)
 - Service status: systemctl status kiosk.service
 - HDMI config: ${CONFIG_TXT}
 - Console niet dimmen: ${CMDLINE_TXT}

Aanpassen van de HTML? Bewerk: ${INDEX_FILE}

TIP: Ondersteboven i.p.v. links↔rechts?
     sudo systemctl stop kiosk.service
     sudo sed -i 's/--reflect x/--reflect y/' ${XINITRC}
     sudo systemctl start kiosk.service

Als je geen goed beeld krijgt: zet vaste resolutie in ${CONFIG_TXT}
 (hdmi_group=1 en hdmi_mode=16 voor 1080p60) en reboot.

Reboot aanbevolen: sudo reboot
========================================
EOT
}
